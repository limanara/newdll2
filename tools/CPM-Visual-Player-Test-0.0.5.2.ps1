...
Write-Host "CPM 0.0.5.2: Player visual $playerId conectado." -ForegroundColor Green
Write-Host "O NPC sera criado perto da posicao atual do seu personagem." -ForegroundColor Cyan
Write-Host "Fase 1: linha reta. Fase 2: circulo. Teste de 45 segundos." -ForegroundColor Cyan
...
for ([uint32]$sequence = 0; $sequence -lt 900; $sequence++) {
    ...
    if ($sequence -lt 200) {
        # Dez segundos em linha reta, facilitando confirmar deslocamento.
        [single]$x = $sequence * 0.025
        [single]$y = 0.0
        [single]$yaw = 0.0
    } else {
        # Depois, circulo de quatro metros para validar posicao e rotacao.
        $angle = ($sequence - 200) * 0.025
        [single]$x = 5.0 + ([Math]::Cos($angle) * 4.0)
