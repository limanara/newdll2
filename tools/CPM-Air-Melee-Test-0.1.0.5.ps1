...
if($welcome.Length-ne 16-or[BitConverter]::ToUInt16($welcome,6)-ne 2){throw "Handshake CPM 0.1.0.5 falhou."}
$player=[pscustomobject]@{Udp=$udp;Token=$token;Id=[BitConverter]::ToUInt32($welcome,12);Clock=[Diagnostics.Stopwatch]::StartNew()}

Write-Host "CPM 0.1.0.5: Native Air + Equip + Melee iniciado." -ForegroundColor Green
...
for([uint32]$seq=0; $seq -lt 1520; $seq++){
    ...
    elseif($seq -lt 1000){$phase="RETORNO EM PE";$x=29.7;$yaw=180;$speed=0.0;$loc=0;$detail=1}
    ...
    elseif($seq -lt 1380){$phase="GUARDANDO ARMA";$x=2.2;$yaw=180;$flags=0;$upper=0;$weaponState=0;$meleeState=22}
    elseif($seq -lt 1480){$phase="ATAQUE CORPO A CORPO";$x=2.2;$yaw=180;$flags=0;$upper=0;$weaponState=0;$meleeState=11;if($seq -eq 1380 -or $seq -eq 1430){$melee++}}
...
$udp.Dispose();Write-Host "Teste 0.1.0.5 finalizado. O NPC deve desaparecer apos o timeout." -ForegroundColor Green
