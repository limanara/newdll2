...
if($welcome.Length-ne 16-or[BitConverter]::ToUInt16($welcome,6)-ne 2){throw "Handshake CPM 0.1.0.6 falhou."}
$player=[pscustomobject]@{Udp=$udp;Token=$token;Id=[BitConverter]::ToUInt32($welcome,12);Clock=[Diagnostics.Stopwatch]::StartNew()}

Write-Host "CPM 0.1.0.6: Real Z + NPC Air + Return + Melee iniciado." -ForegroundColor Green
...
for([uint32]$seq=0; $seq -lt 1420; $seq++){
    ...
    elseif($seq -lt 520){$phase="PARADO ANTES DO SALTO";$x=25.8}
    elseif($seq -lt 560){$phase="SALTO";$x=25.8+($seq-520)*0.04;$z=1.45*([single]($seq-520)/40.0);$speed=2.0;$detail=18}
    elseif($seq -lt 600){$phase="QUEDA";$x=27.4+($seq-560)*0.04;$z=1.45*(1.0-[single]($seq-560)/40.0);$speed=2.0;$detail=14}
    elseif($seq -lt 660){$phase="ATERRISSAGEM";$x=29.0;$z=0.0;$detail=23}
    elseif($seq -lt 900){$phase="RETORNO CORRENDO";$x=29.0-(25.0*[single]($seq-660)/240.0);$yaw=180;$speed=5.5;$loc=2;$detail=4}
    elseif($seq -lt 960){$phase="PARADO APOS RETORNO";$x=4.0;$yaw=180;$speed=0.0;$loc=0;$detail=1}
    elseif($seq -lt 1020){$phase="EQUIPANDO ARMA";$x=4.0;$flags=1;$weaponState=5}
    elseif($seq -lt 1120){$phase="MIRA E TIROS";$x=4.0;$yaw=180;$flags=3;$upper=6;$weaponState=5
        if($seq -eq 1040 -or $seq -eq 1070 -or $seq -eq 1100){$shot++;$weaponState=8}}
    elseif($seq -lt 1180){$phase="RECARGA";$x=4.0;$yaw=180;$flags=1;$upper=3;$weaponState=2;if($seq -eq 1120){$reload++}}
    elseif($seq -lt 1260){$phase="GUARDANDO ARMA";$x=2.2;$yaw=180;$flags=0;$upper=0;$weaponState=0;$meleeState=22}
    elseif($seq -lt 1360){$phase="ATAQUE CORPO A CORPO";$x=2.2;$yaw=180;$flags=0;$upper=0;$weaponState=0;$meleeState=11;if($seq -eq 1260 -or $seq -eq 1310){$melee++}}
...
$udp.Dispose();Write-Host "Teste 0.1.0.6 finalizado. Todas as fases foram enviadas; o NPC desaparecera apos o timeout." -ForegroundColor Green
