...
if($welcome.Length-ne 16-or[BitConverter]::ToUInt16($welcome,6)-ne 2){throw "Handshake CPM 0.1.0.2 falhou."}
$player=[pscustomobject]@{Udp=$udp;Token=$token;Id=[BitConverter]::ToUInt32($welcome,12);Clock=[Diagnostics.Stopwatch]::StartNew()}

Write-Host "CPM 0.1.0.2: Action Controller iniciado." -ForegroundColor Green
Write-Host "Cada fase sera anunciada e mantida tempo suficiente para observacao." -ForegroundColor Cyan
[uint32]$shot=0;[uint32]$reload=0;[uint32]$melee=0;$lastPhase=""

for([uint32]$seq=0; $seq -lt 1200; $seq++){
    ...
    elseif($seq -lt 820){$phase="EQUIPANDO ARMA";$x=29.7;$flags=1;$weaponState=5}
    elseif($seq -lt 980){$phase="MIRA E TIROS";$x=29.7;$yaw=35;$flags=3;$upper=6;$weaponState=5
        if($seq -eq 860 -or $seq -eq 900 -or $seq -eq 940){$shot++;$weaponState=8}}
    elseif($seq -lt 1060){$phase="RECARGA";$x=29.7;$yaw=35;$flags=1;$upper=3;$weaponState=2;if($seq -eq 980){$reload++}}
    elseif($seq -lt 1160){$phase="ATAQUE CORPO A CORPO";$x=29.7;$flags=1;$meleeState=11;if($seq -eq 1060 -or $seq -eq 1110){$melee++}}
    else{$phase="FINAL PARADO";$x=29.7}
...
$udp.Dispose();Write-Host "Teste finalizado. O NPC deve desaparecer apos o timeout." -ForegroundColor Green
